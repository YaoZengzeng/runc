 ...
 */
// 从环境变量中获取init pipe fd，它可以被用于读取bootstrap data
// 并且在我们配置好环境之后，告诉父进程新的pid
...
void join_namespaces(char *nslist)
{
	 ...
	 * access the paths.
	 * 我们需要先打开文件描述符，因为我们一旦加入mnt namespace之后
	 * 我们就不能访问这些paths了
	 */
	do {
		int fd;
	 ...
	 * 加入namespace的顺序是非常重要的，我们总是应该先加入user namespace
	 */
...
void nsexec(void)
{
	...
	switch (setjmp(env)) {
		 ...
		 *          process.
		 *			我们是父进程，我们的工作是创建一个子进程stage 1，并且写入uid_map和gid_map
		 *			这个进程之后会接着去创建新的进程，之后它会发送给我们它的pid，接着我们会将它发
		 *			送给bootstrap进程
		 */
	case JUMP_PARENT:{
			int len;
			 ...
			 * 父进程只有在子进程和孙子进程都准备好的情况下，父进程才会返回
			 * 这样我们就能获取到所有子进程的error codes
			 */
			while (!ready) {
				enum sync_t s;
				int ret;
			 ...
			 * 我们需要将init_func的pid和第一个子进程的pid返回给父进程
			 * 我们两者都需要返回，因为我们不能reap我们创建的第一个子进程
			 * 这时候就需要我们的父进程来对它进行处理
			 */
		 ...
		 *			我们是第一个子进程，我们的工作是加入netlink payload中提供的namespace
		 *			unshare所有请求的namespaces，如果我们要求CLONE_NEWUSER，我们会请求
		 *			stage 0的父进程为我们设置user mapping。接着，我们会创建一个新的子进程
		 *			stage 2用于PID namespace，之后我们会将子进程的PID发送到父进程
		 */
	case JUMP_CHILD:{
			pid_t child;
			enum sync_t s;
			 ...
			 * 因为PID namespace，我们需要再次fork，setns或者unshare不好改变调用进程的PID namespace
			 * 因为这个改变调用者已有的概念，这可能会导致一些应用和库的崩溃
			 * 因此我们必须fork，从而真正进入一个新的PID namespace
			 */
		 ...
		 * 这是最后一个子进程，只有它能返回到Go runtime，本进程的工作就是完成一些最后的清理并且
		 * 返回到Go runtime，让init_linux.go运行
		 */
	case JUMP_INIT:{
			/*
			...
			// 等待父进程和孙子进程的同步
			// 从管道中读取父进程发送来的SYNC_GRANDCHILD信号
			if (read(syncfd, &s, sizeof(s)) != sizeof(s))
